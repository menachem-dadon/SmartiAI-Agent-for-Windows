...
start_idx = content.find("    def _schedule_background_task_thread(self, task):")
if start_idx == -1:
    print("Could not find start of _schedule_background_task_thread")
    exit(1)

end_idx = content.find("    def _update_tools_config_from_files(self):")
if end_idx == -1:
    print("Could not find end of _schedule_background_task_thread")
    exit(1)
...
line_ending = "\r\n" if "\r\n" in content else "\n"
new_content = content[:start_idx] + replacement_schedule.replace("\n", line_ending) + content[end_idx:]

with open(file_path, "w", encoding="utf-8") as f:
    f.write(new_content)

print("Successfully replaced _schedule_background_task_thread!")
